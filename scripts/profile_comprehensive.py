...
async def profile_http_server(
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        ...
        async with session.post(f"{base_url}/eth/v1/keystores", json=payload, headers=headers) as response:
        ...
        for i in range(num_requests):
            ...
            async with session.post(
                ...
                if "_profile" in data:
                    ...
                    for phase in ["http_parsing", "json_parsing", "auth_check", 
                                  "domain_computation", "key_lookup", "bls_signing", 
                                  "response_encoding"]:
                        if phase in profile:
...
def print_report(raw_stats, python_stats, http_stats):
    ...
    print("-" * 60)
    total_measured = 0
    for phase_name in ["http_parsing", "json_parsing", "auth_check", 
                       "domain_computation", "key_lookup", "bls_signing", "response_encoding"]:
    ...
    if 'total_client' in http_stats and 'total_server' in http_stats:
        ...
        print(f"  Network + framework:  {network_overhead:6.1f} µs ({network_overhead/client_total*100:.1f}%)")
