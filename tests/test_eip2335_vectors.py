...
class TestCipherSupport:
    ...
    def test_aes_256_ctr_support(self, sample_keystore) -> None:
    ...
    def test_unsupported_cipher_rejected(self) -> None:
        ...
        keystore_data = EIP2335_SCRYPT_KEYSTORE.copy()
        ...
        assert "unsupported" in str(exc_info.value).lower() or "cipher" in str(exc_info.value).lower()
...
class TestChecksumVerification:
    ...
    def test_checksum_failure_modified_ciphertext(self) -> None:
        ...
        with pytest.raises(KeystoreError) as exc_info:
            keystore.decrypt(EIP2335_PASSWORD)
        assert "password" in str(exc_info.value).lower() or "checksum" in str(exc_info.value).lower()
